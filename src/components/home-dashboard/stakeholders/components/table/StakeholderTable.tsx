...
import { MoreHorizontal, Pencil, Trash2, Check, Building2 } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export const StakeholderTable = ({
  ...
  return (
    <div className="w-full animate-in fade-in duration-500 pb-8">
          ...
          <TableBody className="divide-y divide-gray-50">
            {data.map((item) => (
              ...
              >
                ...
                <TableCell className="py-5 px-6 text-gray-600 text-sm font-normal">
                  {item.tipo}
                </TableCell>

                <TableCell className="py-5 px-6 text-gray-400 text-sm font-normal">
      ...
      {/* Rodapé Externo conforme solicitado */}
      <div className="py-6 text-left">
        <span className="text-[13px] font-normal text-slate-400">
          Mostrando {data.length} de {data.length} stakeholders
        </span>
      </div>
    </div>
  );
