...
export const SwotItemModal = ({
  ...
  // Agora o TS reconhece esses campos automaticamente!
  ...
  useEffect(() => {
    const total = Number(imp) * Number(int) * Number(ten);
    setValue("pontuacaao", total);
  }, [imp, int, ten, setValue]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[550px] rounded-[32px] p-0 overflow-hidden border-none shadow-2xl bg-white animate-in fade-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
          ...
          <div className="bg-emerald-50/50 border border-emerald-100/50 rounded-2xl p-6 flex flex-col items-center justify-center space-y-1">
            ...
            <span className="text-5xl font-black text-emerald-500">
              {Number(imp) * Number(int) * Number(ten)}
            </span>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 h-12 border border-slate-200 rounded-xl font-bold text-slate-500 hover:bg-slate-50 transition-all text-sm"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="flex-1 h-12 bg-[#10b981] text-white rounded-xl font-bold hover:bg-[#0da673] shadow-lg shadow-emerald-100 transition-all text-sm active:scale-95"
            >
              Salvar Item
            </button>
          </div>
