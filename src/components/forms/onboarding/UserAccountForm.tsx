...
import { Check, Eye, EyeOff, CheckSquare } from "lucide-react";
...
// Importando os tipos do seu schema (Certifique-se de que o nome está correto no seu arquivo de schemas)
import { accountCreationSchema, type AccountCreationData } from "@/lib/validators/schema";

export const UserAccountForm = () => {
  const [showPassword, setShowPassword] = React.useState(false);
  
  // Tipagem do useForm com AccountCreationData para remover erros de inferência
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<AccountCreationData>({
    resolver: zodResolver(accountCreationSchema),
  });

  // Removido o 'any' do erro. Agora aceita o tipo específico do FieldError ou undefined
  ...
  // Tipagem explícita dos dados no onSubmit
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-gray-50/50 rounded-xl p-6 space-y-3 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
            <p className="text-gray-500">Empresa: <span className="text-gray-900 font-medium italic">limao com mel</span></p>
